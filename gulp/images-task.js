...
module.exports = function imagesTask(project, args) {
    ...
    return imagesStream.pipe(shell([ 'attrib -r "<%= file.path %>"', ])) // remove readonly attribute, since we're overwriting the image
                       .pipe(imagemin([
                           imagemin.optipng({ optimizationLevel: maximumPngOptimizationLevel, }),
                           imagemin.jpegtran({ progressive: true, }),
                           imagemin.gifsicle({ interlaced: true, optimizationLevel: maximumGifOptimizationLevel, }),
                           imagemin.svgo({ plugins: [ { removeViewBox: false, }, { removeUselessDefs: false, }, ], }),
                       ]))
